...
func TestDiff_mysql(t *testing.T) {
	...
	defer func() {
		_, err := d.Exec(
			cbg,
			frag.Query("DROP TABLE IF EXISTS ?", builder.TFrom(&testdata.User{})),
		)
	...
	f_is_member TINYINT NOT NULL DEFAULT 0,
	f_real_name TEXT NOT NULL,
	f_username VARCHAR(255) NOT NULL,
	f_nick_name VARCHAR(128) NOT NULL DEFAULT '',
	f_age INT NOT NULL,
...
CREATE INDEX i_nickname ON t_user (f_nick_name,f_deleted_at);`,
		...
		_, err := d.Exec(cbg, actions)
		testx.Expect(t, err, testx.Succeed())
	})

	t.Run("MigrateUserV1_2nd", func(t *testing.T) {
		cata, err := d.Catalog(cbg)
		testx.Expect(t, err, testx.Succeed())
		curr := cata.T("t_user")
		next := builder.TFrom(&testdata.User{})

		actions := diff.Diff(cbg, d.Dialect(), curr, next)
		q, _ := frag.Collect(cbg, actions)
		t.Log("\n" + q)

		testx.Expect(t, actions.IsNil(), testx.BeTrue())
	})
